...
if ((int)$_SERVER['CONTENT_LENGTH'] > $multiplier*(int)$POST_MAX_SIZE && $POST_MAX_SIZE) {
	...
	echo "POST превысил максимально допустимый размер.";
	exit(0);
}

// Settings
// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
$save_path = $_SESSION['swf_save_path'] ? $_SESSION['swf_save_path'] : getcwd()."/uploads/";
...
$uploadErrors = array(
	...
	0=>"Ошибок нет, файл успешно загружен",
	1=>"Загружаемый файл превысил директиву upload_max_filesize в php.ini",
	2=>"Загружаемый файл превысил директиву MAX_FILE_SIZE, специфичную для HTML формы",
	3=>"Загружаемый файл частично загружен",
...
if (!isset($_FILES[$upload_name])) {
	...
	HandleError("Файл не имеет названия.");
	exit(0);
}
...
if (!$file_size || $file_size > $max_file_size_in_bytes) {
	HandleError("Размер файла превышает максимально допустимый размер");
	exit(0);
}
...
if (strlen($file_name) == 0 || strlen($file_name) > $MAX_FILENAME_LENGTH) {
	HandleError("Неверное имя файла");
	exit(0);
}
...
if (file_exists($save_path . $file_name)) {
	HandleError("Файл с таким именем уже существует");
	exit(0);
}
...
if(@$extension_whitelist) 
{
	...
	{
		HandleError("Неверное расширение файла");
		exit(0);
	}
}
...
if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
	HandleError("Файл не может быть сохранен: ". $save_path.$file_name);
	exit(0);
}

// Return output to the browser (only supported by SWFUpload for Flash Player 9)

echo "Файл получен ". $save_path.$file_name;
exit(0);
